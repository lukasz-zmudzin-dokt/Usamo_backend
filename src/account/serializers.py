...
class AbstractAccountSerializer(serializers.ModelSerializer):
    tac_accepted = serializers.BooleanField(default=False, write_only=True)
    ...
    def validate_tac_accepted(self, value):
        if value == False:
...
class DefaultAccountSerializer(AbstractAccountSerializer):
    ...
    class Meta:
        ...
        fields = ['email', 'username', 'last_name', 'first_name',
                  'password', 'phone_number', 'facility_name', 'facility_address', 'picture_url', 'tac_accepted']
        extra_kwargs = {
    ...
    def update(self, instance, validated_data):
        ...
        if account_data:
            ...
            if new_address_data: 
                serializer = AddressSerializer(def_account.facility_address, data=new_address_data, partial=True)
                serializer.update(def_account.facility_address, new_address_data)
...
class EmployerAccountSerializer(AbstractAccountSerializer):
    ...
    class Meta:
        model = Account
        fields = ['email', 'username', 'last_name', 'first_name',
                  'password', 'phone_number', 'company_name', 'company_address', 'nip', 'picture_url', "tac_accepted"]
        extra_kwargs = {
            'password': {'write_only': True},
            'email': {'required': True},
            'last_name': {'required': True},
    ...
    def update(self, instance, validated_data):
        ...
        if account_data:
            ...
            if new_address_data:
                serializer = AddressSerializer(employer_account.company_address, data=new_address_data, partial=True)
                serializer.update(employer_account.company_address, new_address_data)
            phone_number = account_data.pop('phone_number', None)
            if phone_number:
...
class StaffAccountSerializer(AbstractAccountSerializer):
    ...
    class Meta:
        ...
        fields = ['email', 'username', 'first_name', 'last_name',
                'password', 'group_type', 'role', 'picture_url', 'tac_accepted']
        extra_kwargs = {
            'password': {'write_only': True},
            'email': {'required': True},
            'last_name': {'required': True},
