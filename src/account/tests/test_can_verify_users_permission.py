...
class CanVerifyUsersPermissionTest(unittest.TestCase):

    def setUp(self):
        self.permission = CanStaffVerifyUsers()
        self.request = Mock()
        self.view = Mock()
    ...
    def test_user_is_staff_and_can_verify_users(self):
        groups = Mock()
        groups.filter(name=StaffType.STAFF_VERIFICATION.value).exists.return_value = True
    ...
    def test_user_is_staff_and_can_not_verify_users(self):
        groups = Mock()
        groups.filter(name=StaffType.STAFF_VERIFICATION.value).exists.return_value = False
